...
#------------------------network setting---------------------
        self.weights ={
            ...
            'wc5_2': tf.Variable(tf.truncated_normal([1, 1, 512, 20], stddev=0.01), name='wc5_2'),
        }
        self.biases = {
            ...
            'bc5_2': tf.Variable(tf.constant(0.0, shape=[20]), name='bc5_2'),
        }
        self.build_model()
    ...
    def build_model(self):
        ...
        self.fake_B = tf.argmax(self.fake_B, dimension=3)
        self.fake_B = tf.to_float(tf.expand_dims(self.fake_B, dim=3))

        self.real_AB = tf.concat([self.real_B, self.real_A], 3)
        self.fake_AB = tf.concat([self.fake_B, self.real_A], 3)
    ...
    def load_random_samples(self, set_name):
        ...
        for batch_file in data:
            g_, d_ = load_lip_data_t2(batch_file, 'train')
            batch_g.append(g_)
            batch_d.append(d_)
    ...
    def test(self, args):
        ...
        for idx in xrange(int(len(sample_files) / self.batch_size)):
            ...
            for batch_file in sample_i:
                g_ = load_lip_data_t2(batch_file, 'test')
                batch_g.append(g_)
